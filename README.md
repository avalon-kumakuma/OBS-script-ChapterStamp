...
- タイムスタンプのファイルはOBS Studioの設定 録画パスに格納されます。  
  ファイル名は書式設定で『年/月/日/時/分/秒』の部分のみ取得開始時間に置き換えて、後尾に**Chapter_Stamp.txt**を付与します。  
  書式設定の初期設定値では上書きされる事はありませんが、変更れている場合は『年/月/日/時/分/秒』以外の書式は無視されますので、
  生成ファイル名が重ならないよう、区別できる書式設定に変更して下さい。  

            初期設定値時のファイル名：2024-01-01 17-05-59 Chapter_Stamp.txt

> [!IMPORTANT]  
> 表示精度と内部実装の関係で、タイムスタンプやファイル名で1秒ほど誤差が出る場合があります。  
> 配信では通信状況などの遅延が出た場合もライブアーカイブとタイムスタンプにズレが発生します。  

- メディアソース(STAMP_BEEP)を単一のトラックに録音する事で、マルチトラック対応の動画編集ソフトを利用する事で
  BEEP音トラックの波形を編集ポイントの目印に利用出来ます。[^4][^5]  

> [^4]: 連続再生可能なタイミングなどはメディアソース再生の仕様に準拠します。  
> [^5]: タイムスタンプとメディアソース再生はズレがほぼ有りませんが、イベント発生時と再生にはズレ生じますのであくまで目安です。  
...
> 『統計情報 出力』設定を有効にするとAbout 統計情報を終了時にタイムスタンプテキストの下部に出力します。
...
- 【タイムスタンプ】
    ...
    - 第１項目は時間をYoutubeチャップター形式のタイムスタンプを表記します。  
          またYouTube制限の前タイムスタンプとの間隔が10秒未満の場合に先頭『\*』マークがつきます。  
    ...
    - 第４項目は録画開始・スクリーンショット取得・リプレイバッファ取得のイベントのみ、  
          取得ファイル名を表記します。  
          ***※タイムスタンプとファイル名の時間にズレが発生する場合があります。***  
    - 第５項目はYouTube URL最後尾に追加すると該当シーンが再生開始ポジションになる、  
          時間指定のクエリパラメータを表記します。  
          ライブアーカイブとタイムスタンプには誤差がありますので、目安としてご利用下さい。
...
- About 統計情報 出力例
    ...
    |Dropped frame rate: 0.00%                      |ドロップフレームの割合|
...
<img src="./images/OBS_Script_version_error.png" width="50%" alt="バージョンエラー時の表示"></br>
  ...
  - Ｑ、ファイル名やタイムスタンプの時間、またはSTAMP_BEEP音が少しズレてしまう。  
    - 申し訳ありません、OBS StudioのLuaスクリプトの仕様で非同期で動作する関係で、
      ...
      ご利用PCの性能及び使用状況で負荷が高い場合は発生しやすくなりますので、OBS Studioの
      処理が滞らないようにPCの負荷を下げて使用率に余裕を持たせてご利用ください。  

  - Ｑ、Youtubeライブ アーカイブとタイムスタンプの時間がズレてしまう。  
    - どうしても利用環境による処理遅延などがあり、仕様で必ずズレてしまいます。  
      全てのズレに対応が困難な為、申し訳ありませんが手動にてタイムスタンプを修正対応して
      ご利用ください。  
      遅延が一定の場合は、テキスト内のタイムスタンプ時間部分をスプレッドシートや
      エクセルなどの表計算ソフトを利用して一定の加算処理をするようにして対処してください。  
      該当ライブアーカイブのURLへタイムスタンプ行の第５項目のクエリパラメータを最後尾へ
      付加してアクセスする事で該当時間から再生になりますので、
  ...
  - Ｑ、カスタムホットキー毎に別の音を鳴らしたい
    - スクリプト上では利用が複雑化してしまう為、対応が難しいく申し訳ありません。
      ...
      **【ホットキー別に音を変更する手順】**
      ...
      5. 『***STAMP_BEEP***』の再生音が鳴らないように、Chapter Stamp設定で該当の『BEEP再生：』を無効にして下さい。  

      シーンチェンジ・スクリーンショット取得・リプレイバッファ取得も、同様にメディアソースと同じキーコマンドを
